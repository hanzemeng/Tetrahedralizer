...
//
//std::pair<std::vector<uint32_t>, std::vector<uint32_t>> InteriorCharacterizationHandle::calculate(Polyhedralization& polyhedralization, std::vector<uint32_t>& constraints, std::vector<double3>& approximated_vertices, std::vector<std::vector<uint32_t>>& coplanar_triangles, double polyhedron_in_multiplier)
//{
//    vector<chrono::steady_clock::time_point> times;
//    times.push_back(chrono::steady_clock::now());
//    
...
//                    if(polyhedralization.m_facets[f].intersects_coplanar_triangle(c0, c1, c2, ignore_axis, polyhedralization.m_vertices, polyhedralization.m_segments))
//                    {
...
//                    
//                    shared_ptr<genericPoint> centroid = polyhedralization.m_facets[f].get_implicit_centroid(polyhedralization.m_vertices);
//                    int orient = genericPoint::orient3D(*polyhedralization.m_vertices[c0], *polyhedralization.m_vertices[c1], *polyhedralization.m_vertices[c2], *centroid);
////                    int orient = 0;
////                    polyhedralization.m_facets[f].get_vertices(polyhedralization.m_segments, Facet::m_get_vertices_res);
////                    for(uint32_t v : Facet::m_get_vertices_res)
////                    {
...
//    }
//    
//    return make_pair(polyhedrons_labels, facets_centroids_mapping);
//}
