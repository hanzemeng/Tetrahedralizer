...
public:
    ...
    std::vector<uint32_t> order_facets(std::vector<std::shared_ptr<genericPoint>>& vertices, std::vector<double3>& approximated_vertices, std::vector<Segment>& segments, std::vector<Facet>& facets, std::vector<uint32_t>& facets_coplanar_groups);
    void order_facets(std::vector<uint32_t>& all_facets_indexes, std::vector<std::shared_ptr<genericPoint>>& vertices, std::vector<double3>& approximated_vertices, std::vector<Segment>& segments, std::vector<Facet>& facets, std::vector<uint32_t>& facets_coplanar_groups,
...
private:
    ...
    int check_plane_facet_intersection(uint32_t c, uint32_t nc,std::vector<std::shared_ptr<genericPoint>>& vertices, std::vector<Segment>& segments,std::vector<Facet>& facets, std::vector<uint32_t>& facets_coplanar_groups, std::vector<std::pair<double3,double>>& facets_spheres, std::unordered_map<uint32_t, std::pair<double3,double>>& planes_equations);
