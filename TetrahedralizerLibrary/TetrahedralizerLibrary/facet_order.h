...
//    }
    ...
    order_tree[res].top = order_facets(best_top, vertices, segments, facets, order_tree, constraints_facets_cache, constraints_vertices_cache);
    order_tree[res].bot = order_facets(best_bot, vertices, segments, facets, order_tree, constraints_facets_cache, constraints_vertices_cache);
    return res;
}
...
inline std::vector<FacetOrder> order_facets(std::vector<std::shared_ptr<genericPoint>>& vertices, std::vector<Segment>& segments, std::vector<Facet>& facets)
{
    ...
    order_facets(facets_indexes, vertices, segments, facets, res, constraints_facets_cache, constraints_vertices_cache);
    return res;
}
