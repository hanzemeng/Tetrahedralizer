...
inline void polygon_plane_intersection_helper(uint32_t& i0,uint32_t& i1,uint32_t e)
{
...
inline void polygon_plane_intersection(uint32_t c0,uint32_t c1,uint32_t c2,uint32_t t0,uint32_t t1,uint32_t t2, vector<genericPoint*>& vertices, vector<Edge>& edges)
{
    if(0 == edges.size())
    {
        return;
    ...
    for(uint32_t i=0; i<edges.size(); i++)
    {
        ...
        else if(1 == o0)
        {
            ...
            else
            {
                ...
                {
                    vertices.push_back(new implicitPoint3D_LPI(vertices[edges[i].p0]->toExplicit3D(),vertices[edges[i].p1]->toExplicit3D(),
...
inline int triangle_tetrahedron_intersection(genericPoint* pc0,genericPoint* pc1,genericPoint* pc2,genericPoint* pt0,genericPoint* pt1,genericPoint* pt2,genericPoint* pt3)
{
    int res = 0;
    ...
    EXIT:
    for(uint32_t i=7; i<vertices.size(); i++)
    {
        delete vertices[i];
    }
    return res;
