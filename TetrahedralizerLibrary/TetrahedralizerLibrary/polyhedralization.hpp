...
public:
    Polyhedralization();
    Polyhedralization(uint32_t polyhedrons_count, uint32_t* polyhedrons);
    Polyhedralization(const vector<vector<uint32_t>>& polyhedrons);

    void assign_polyhedrons(const vector<vector<uint32_t>>& polyhedrons);
